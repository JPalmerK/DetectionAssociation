function [value] = KACRevisedXcorr(app, file_listTable,Hyddelay, noisebuffpts, ...
...
% Window name and length
winName = app.WindowNameDropDown.Value
winLen = app.WindowLengthEditField.Value


eventDuration = (callStopSamp-callStartSamp)/fs; 

% some of Shyams detections are too damn short
if eventDuration<2

    % get the difference in samples
    missingSamps = round((2-eventDuration)/2*fs);
    
    % make the detection larger for cross correlation
    callStopSamp=callStopSamp+missingSamps;
    callStartSamp=callStartSamp-missingSamps;
...
% duration is always
totalpts = ceil(eventDuration * fs) + (eventDuration * event_pad);
...
% first and last event could be different size, resize to confirm
totalpts = size(sigdata,1);

% DC offset per channel
sigdata = sigdata - ones(size(sigdata,1), 1) * mean(sigdata);
...
win_func = feval(get_taper_func(winName), winLen);
win_scale = sum(win_func.^2) / winLen;
...
[ref_eventspcs] = specgram(sig, winLen, fs, win_func, ovrlap_pts);


num_spc = size(ref_eventspcs, 2);

% get total number of points used for spectrogram calculation
Ntotal = ((num_spc - 1) * (winLen - ovrlap_pts)) + winLen;
...
ref_eventspcs = ref_eventspcs / winLen;


% get background noise
sig = [end_pad; sigdata(noiseinx, ref_chan); end_pad];
[noisespcs] = specgram(sig,winLen, fs, win_func, ovrlap_pts);
...
ref_rms_intensity = ref_rms_press^2 / (1023.6 * 1430); %xxx question



% get ref event spectrogram without end padding
[ref_eventspcs] = specgram(sigdata(eventinx, ref_chan), winLen, fs, win_func, ovrlap_pts);
% bandlimit
...
refeventspcs_blm = standardize_mat(app, refeventspcs_blm);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for inx = 1:num_chan
    ...
    xeventspc{inx} = specgram(sigdata(xchaninx{inx}, inx), winLen, fs, win_func, ovrlap_pts);
    ...
    xeventspc_blm{inx} = standardize_mat(app, xeventspc_blm{inx});
...
fftsz = winLen; %% # of bins for parsing the frequency range
